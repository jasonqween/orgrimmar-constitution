...
Когда выходит новая модель (обновление существующей или новая от провайдера).

##### 6.1 Кто инициирует

| Роль | Как узнаёт | Действие |
|------|------------|----------|
| DevOps (Иллидан) | Мониторинг релизов провайдеров | Задача MODEL-UPGRADE в inbox |
| Coder (Тралл) | Обнаружил при работе / принц сообщил | Задача MODEL-UPGRADE в inbox |
| Принц | Сам решил | Задача напрямую |

##### 6.2 Этапы смены

| Фаза | Действие | Кто | Результат |
|------|----------|-----|-----------|
| **DETECT** | Зафиксировать выход модели: название, провайдер, стоимость, контекст | DevOps / Coder | Задача в inbox |
| **EVALUATE** | Тест на 3--5 типовых задачах целевого агента | Coder | Отчёт: качество, скорость, цена |
| **COMPARE** | Таблица: новая vs текущая (качество / скорость / цена / контекст) | Coder | Рекомендация |
| **PROPOSE** | PR в конституцию: обновить каталог + модельную карту | Coder | PR с обоснованием |
| **APPROVE** | Принц ревьюит PR | Принц | Merge или отклонение |
| **CANARY** | Раскатка на 1 агента (наименее критичного), 24--48ч | Coder + DevOps | Отчёт по canary |
| **ROLLOUT** | Раскатка на остальных. Порядок: Illidan -> Thrall -> Sylvanas | Coder | Конфиги обновлены |
| **VERIFY** | Мониторинг 48ч после полной раскатки | DevOps | Финальный отчёт |

##### 6.3 Откат

- Бэкап конфига **обязателен** перед каждой сменой модели
- Откат на предыдущую модель -- за 5 мин (замена конфига + рестарт)
- Триггеры отката: рост ошибок >10%, деградация качества, перерасход бюджета
- Откат не требует одобрения принца (auto-allowed для coder/devops)

##### 6.4 Минорные обновления

Если провайдер обновляет модель без смены ID (патч, улучшение):
- Не требует полного цикла DETECT -> VERIFY
- Coder/DevOps мониторят качество 24ч
