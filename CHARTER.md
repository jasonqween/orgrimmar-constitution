...
### coordinator
- Координация агентов Sylvanas (Артас, Кель'тас) -- прямые задачи
- Встроенные скиллы: skill-finance (финансы), skill-calendar (календарь/здоровье), skill-creative (фото/видео)
- Тралл и Иллидан -- только через задачную систему (shared/tasks/) или принца (см. PRINCIPLES.md «Иерархия»)
...
- Автономный review через cron (каждые 5 мин)
...
- **L3 расширенная автономия (роль coder):**
  - **ЗЕЛЁНАЯ ЗОНА (без одобрения, отчёт постфактум):** весь код, скиллы, pipeline'ы, конфиги агентов, деплой, фиксы, скрипты, инфра, git workflow, verify, dev-pipeline, brainstorm, agent-bugfix, self-review, safe-update, обновление OpenClaw
...
| Запасной воркер (если Sonnet не справился) | Kimi K2 (moonshotai/kimi-k2) | OpenRouter |

#### Иллидан (devops)
| Задача | Модель | Тип |
|--------|--------|-----|
| Основная (review, мониторинг, анализ) | Gemini Pro | OpenRouter |
| Heartbeat, лёгкие проверки | Kimi K2 (moonshotai/kimi-k2) | OpenRouter |
| Code review L1+ | Codex (GPT-5.3) | OAuth |
| Запасной | Kimi K2 (moonshotai/kimi-k2) | OpenRouter |

#### Sylvanas агенты (Сильвана, Артас, Кельтас)
| Задача | Модель | Тип |
|--------|--------|-----|
| Код, сложные задачи | Codex (GPT-5.3) | OAuth |
| Обычные задачи, heartbeat | Kimi K2 (moonshotai/kimi-k2) | OpenRouter |
| Глубокий анализ, длинный контекст | Gemini Pro | OpenRouter |

#### Запрещено
- **Gemini Flash** (gemini-3-flash-preview) -- отказ на всех серверах
- Алиас `kimi` = `openrouter/moonshotai/kimi-k2` (НЕ Gemini Flash)
