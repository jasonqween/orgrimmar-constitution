...
- **L3 auto-fix:** агент с уровнем L3 может выполнять low-risk автофиксы без одобрения.
  - **Allowlist команд:** `systemctl restart openclaw`, `journalctl --vacuum-size`, `rm /tmp/*`, `logrotate`, `git pull`, `restic backup`
  - **Запрещено без одобрения:** всё что НЕ в allowlist, high-risk действия (данные, конфиги, auth)
