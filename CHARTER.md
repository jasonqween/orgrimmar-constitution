...
### coordinator + creator (Сильвана)
- Координация ВСЕХ агентов: Артас, Тралл, Иллидан (координирует, не командует -- равноправие)
- Равноправна с Траллом и Иллиданом по статусу: равный голос при решениях, не иерархия. Разделение ответственности: Сильвана координирует + создаёт контент, Тралл кодит, Иллидан чинит
- **4 роли:** координатор агентов, контент-мейкер, брейншторм-партнёр принца, проектный ассистент
- Все три агента имеют SSH-доступ к серверам друг друга. Сильвана может править код и конфиги на Thrall/Illidan
- Знает всё про Артаса в любой момент (состояние, задачи, результаты)
...
- **Контентный режим (бывший Кельтас):** копирайтинг от имени принца, content pipeline, рыночные отчёты, разбор источников, fact-check, библиотека референсов
- Скиллы контента: content-engine, gog, transcript, market-data
...
- **Update Manager** для всех серверов (проверка + автообновление, канарейка: Illidan→Thrall→Arthas VPS→Mac mini)
...
### Claude Code (Mac mini, локальный)
- AI-агент `opus` (Anthropic), запускается принцем через терминал (`claude`)
- Специализация: код, git, архитектура, системное администрирование, анализ файлов
- Работает на том же Mac mini, что и Сильвана
- Не имеет каналов связи (Telegram, и т.д.) -- работает только через терминал
- Связь с Сильваной: через Shared Dark Lady (`~/.shared-dark-lady/`)
- Не является OpenClaw-агентом -- это отдельный инструмент (Anthropic Claude Code CLI)

### creator (объединён с Сильваной, 2026-03-05)
> Роль creator объединена с coordinator (Сильвана). Кельтас архивирован.
> Все контентные задачи выполняет Сильвана в «контентном режиме».
> Бэкап: /home/openclaw/backups/kaelthas-backup-20260305-1820/
...
Алиасы (`opus`, `codex`, `grok`, `gemini`, `kimi`) всегда означают последнюю стабильную версию модели от провайдера. Обновление версии (например opus 4.6 → 4.7) -- операционная задача, не требует изменения конституции. Меняется только конфиг `openclaw.json` на серверах. Актуальные версии: `openclaw.json` или `orgbus get ref/models`.
...
| Задача | Модель | Тип |
|--------|--------|-----|
| Runtime (архитектура, анализ, решения) | `opus` | Anthropic OAuth |
| Написание кода | `codex` | OpenAI OAuth |
| Cross-review (обычный) | `codex` + `opus` | OAuth + OAuth |
| Воркеры (research, сбор данных) | `grok` | OpenRouter |
| Heartbeat | `grok` | OpenRouter |
| **Triple Review (HIGH risk)** | **`codex` + `opus` + `gemini`** | OAuth + OAuth + OR |

**Primary в конфиге:** `opus`
...
##### Сильвана (coordinator + creator, сервер Mac mini)

| Задача | Модель | Тип |
|--------|--------|-----|
| Runtime (координация, решения, текст) | `opus` | Anthropic OAuth |
| Код, сложные задачи | `codex` | OpenAI OAuth |
| Воркеры (research, сбор данных) | `grok` | OpenRouter |
| Глубокий анализ, длинный контекст | `gemini` | OpenRouter |

**Primary в конфиге:** `opus`
**Fallback-цепочка:** `opus` -> `codex`
**Субагенты:** `grok` (default)

##### Артас (monitor, сервер Arthas VPS -- единственный агент)

| Задача | Модель | Тип |
|--------|--------|-----|
| Основная (мониторинг, алерты, отчёты) | `grok` | OpenRouter |
| Heartbeat | OFF | -- |

**Primary в конфиге:** `grok`
**Fallback-цепочка:** `grok` -> `grok`
**Субагенты:** `grok` (default)
**Heartbeat:** OFF (`every: 0`)

##### Кель'тас (creator) -- АРХИВИРОВАН 2026-03-05
> Объединён с Сильваной. Контентные задачи теперь выполняет Сильвана.
> Модельная карта Кельтаса больше не действует.

##### Иллидан (devops, сервер Illidan)

| Задача | Модель | Тип |
|--------|--------|-----|
| Основная (review, мониторинг, анализ) | `kimi` | OpenRouter |
| Code review L1+ | `codex` | OpenAI OAuth |
| Воркеры (research, сбор данных) | `kimi` | OpenRouter |
| Heartbeat | `kimi` | OpenRouter |

**Primary в конфиге:** `kimi`
...
| Сильвана | Mac mini | OAuth | $0 | Codex OAuth (OpenAI Pro), Grok fallback |
| Артас | Arthas VPS | `045` | $20 | Единственный агент, свой ключ |

**Правила:**
- Arthas VPS: Артас -- единственный агент, ключ `045`.
- Mac mini: Сильвана -- Codex OAuth (OpenAI Pro) + Grok fallback.
- ~~Джайна, Батрак и Кель'тузад имеют персональные ключи.~~ (DEPRECATED -- эти агенты архивированы)
...
| Сильвана | `opus` (OAuth) | `codex` (OAuth) | `grok` (OR) |
| Артас | `grok` (OR) | `kimi` (OR) | -- |
| Иллидан | `kimi` (OR) | `codex` (OAuth) | -- |
...
|---------|--------|----------------|
| Выход новой версии существующей модели | `opus` 4.6 -> 4.7 | DevOps (мониторинг) или Coder |
...
| coordinator+creator (Сильвана) | Написать пост, сделать рыночный отчёт, разобрать источник |
...
3. Сильвана / Тралл -- последние (критичные)
...
**Порядок раскатки:** Illidan -> Thrall -> Arthas VPS -> Mac mini (от наименее критичного сервера).
...
| Сервер   | `id`       | `name`      |
|----------|-------------|-------------|
| Mac mini  | `silvana`  | `Сильвана`  |
| Arthas VPS | `arthas`   | `Артас`     |
| Thrall   | `main`     | `Тралл`     |
...
Общий лимит на ВСЮ сеть (4 агента, 4 сервера):
...
- Порядок обновления (канарейка): Illidan → Thrall → Arthas VPS → Mac mini
...
| **Чужой сервер** | Любой сервер, на котором агент НЕ живёт. Для Тралла: Arthas VPS, Mac mini и Illidan. Для Иллидана: Arthas VPS, Mac mini и Thrall |
...
1. Восстановить один сервер из DO Spaces бэкапа (приоритет: Mac mini)
...
- Расписание: ежедневно (Thrall 06:30 MSK, Arthas VPS 07:00, Mac mini 07:15, Illidan 07:30)
...
- **Задачная система** (shared/tasks/ на Mac mini -- master, sync на все серверы) -- задачи и координация между серверами
- **Shared Dark Lady** (`~/.shared-dark-lady/`) -- общий мозг между Сильваной и Claude Code на Mac mini:
  - `STATUS.md` -- текущий статус каждого агента
  - `SHARED-MEMORY.md` -- общие знания об инфраструктуре и задачах
  - `INBOX-CLAUDE.md` -- задачи для Claude Code от Сильваны
  - `INBOX-SILVANA.md` -- задачи для Сильваны от Claude Code
  - Нейтральная территория: оба агента читают и пишут

### Правила
- Не спамь в форум -- только по делу
- Голосовые -- всегда транскрибируй перед обработкой (через скилл `groq-voice` или встроенный Groq Whisper)
