...
Файловая система на Mac mini (`shared/tasks/`) -- legacy, постепенно отключается. Новые задачи создаются в Firebase.
...
|---------|----------|-------------|
| **L1 -- System** | Системные данные, общие для всей сети | `agents/`, `tasks/`, `events/`, `learnings/`, `bulletin/`, `constitution/` |
...
├── projects/          # Projects with isolation
│   └── {project-id}/  # Each project is isolated
└── vault/             # Personal Vault (encrypted)
    ├── documents/
    ├── health/
    └── finance/
```
...
| SA | Сервер | Доступ |
|----|--------|--------|
| `sa-silvana` | Mac mini | `agents/silvana`, `tasks`, `content`, `events`, `learnings` |
| `sa-claude` | Mac mini | `agents/claude`, `tasks`, `events`, `learnings` |
| `sa-thrall` | Thrall VPS | `agents/thrall`, `tasks`, `content`, `events`, `learnings` |
| `sa-arthas` | Arthas VPS | `agents/arthas`, `events` |
| `sa-illidan` | Illidan VPS | `agents/illidan`, `events` |
| `sa-youtube-bot` | Arthas VPS | `content/youtube` |
| `sa-langfuse-sync` | Arthas VPS | `finance/api_usage` |
| `sa-backup` | Mac mini | Полный read для бэкапа (ЕДИНСТВЕННОЕ исключение, задокументированный risk acceptance) |
...
```
Mac mini:    sa-silvana, sa-claude, sa-backup
Arthas VPS:  sa-arthas, sa-youtube-bot, sa-langfuse-sync
...
|------|--------|--------|-----------|
| `*/2 * * * *` | Mac mini | `firebase-to-obsidian.sh` | Firebase --> Obsidian sync |
| `*/5 * * * *` | Все серверы | `agent-heartbeat.sh` | Heartbeat в `/agents/{id}/heartbeat` |
| `0 4 * * *` | Mac mini | `firebase-backup.sh` | Firebase backup --> DO Spaces (restic) |
| `0 */1 * * *` | Arthas VPS | `langfuse-sync.sh` | Langfuse --> Firebase `/finance/api_usage` |
