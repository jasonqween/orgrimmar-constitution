...
| Перерасход API > $20/сутки на агента | L3 | Стоп, одобрение принца |
...
### OpenRouter (per-key weekly limits)

| Ключ | Лимит | Роль | Обоснование |
|------|-------|------|-------------|
| thrall | $50/week | coder, воркеры | Основной кодер, спавнит воркеров на Gemini Flash |
| illidan | $30/week | devops | Мониторинг, heartbeat, фиксы |
| silvana | $20/week | coordinator | Triage, отчёты, координация |
| arthas | $15/week | monitoring | Heartbeat, watchdog, алерты |
| kaelthas | $20/week | content | Контент-pipeline |
| jaina | $11/week | finance | Финансовые отчёты |
| batrak | $10/week | routine | Рутинные задачи |
| kelthuzad | $10/week | design | Дизайн |

**Общий бюджет:** ~$166/week (~$720/мес) max теоретический; фактический расход $80-120/мес.

### Правила управления лимитами

1. **Coder (Тралл) может менять лимиты** в пределах: +$20/key, суммарно не более +$50/week без одобрения принца.
2. **Превышение >$20/сутки на агента** -- L3 эскалация (стоп + одобрение принца).
3. **Мониторинг:** cron ежедневно проверяет баланс; алерт при <$10 remaining на любом ключе.
4. **OAuth модели** (Opus, Codex) -- по подписке, без per-token лимитов. Использовать для review и архитектурных задач.
5. **Gemini Flash** -- основная рабочая модель (~70% запросов). $0.10/$0.40 per 1M tokens.

### Подписки (ежемесячные)

| Сервис | Стоимость | Дата списания | Кто управляет |
|--------|-----------|---------------|---------------|
| Claude Max | $200 | 16-е число | Принц |
| ChatGPT Pro | $200 | 18-е число | Принц |
| Cursor | $20 | 20-е число | Принц |
| Claude (Thrall) | $200 | 22-е число | Принц |
| Obsidian | $5 | 22-е число | Принц |
| Perplexity | $20 | 25-е число | Принц |
| **Итого** | **$645/мес** | | |
