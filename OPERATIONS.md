...
**OTP по email** (api-server.py на Timeweb):
- Принц вводит email → получает одноразовый код (через Resend API)
- Код действителен 5 минут, 5 попыток, 60 сек cooldown между отправками
- Список разрешённых email: `/var/www/task-dashboard/allowed_emails.json` (на Timeweb, не в конституции)
- Сессия: 24 часа (SQLite на Timeweb)
- Secrets: хранятся в env Timeweb, никогда не коммитятся
