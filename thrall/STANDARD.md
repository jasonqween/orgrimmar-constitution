...
| **Зачем** | Диск забивается без регулярной чистки. Без чистки /tmp забивается за 2-3 недели (whisper, cron логи, worker артефакты, pipeline results) |
...
| **Модель** | `opus` (исключение, документировано в конституции) |
| **Session** | isolated |
| **Delivery** | announce |
| **Timeout** | 240 сек |
...
Без этих скиллов Тралл не может выполнять свою роль. Удаление = VIOLATION.

### Базовые (конституция, все агенты)

| ID | Скилл | Назначение | Что сломается при удалении |
|----|-------|-----------|---------------------------|
| **THR-SKL-1** | `task-system` | Работа с задачной системой (inbox/active/done) | Не сможет создавать, обновлять, закрывать задачи |
| **THR-SKL-2** | `learnings` | Запись уроков из ошибок в LEARNINGS.md | Ошибки не фиксируются, повторяются |
| **THR-SKL-3** | `memory-tiering` | Управление памятью HOT/WARM/COLD при flush | Память записывается хаотично, файлы раздуваются |
| **THR-SKL-4** | `transcript` | Транскрипция YouTube видео | Не может обрабатывать видео-источники |
| **THR-SKL-5** | `twitter` | Чтение Twitter/X (FxTwitter + SocialData fallback) | Не может читать твиты, треды, статьи |
| **THR-SKL-6** | `memory-audit` | Самоаудит памяти по регламенту конституции | Нарушения памяти не обнаруживаются |

### Ролевые (coder)

| ID | Скилл | Назначение | Что сломается при удалении |
|----|-------|-----------|---------------------------|
| **THR-SKL-7** | `server-ops` | SSH-операции на Sylvanas/Illidan: конфиги, рестарты | Не может управлять другими серверами |
| **THR-SKL-8** | `cross-review` | Ревью PR: change levels, чеклисты, merge | PR от Иллидана остаются без review |
| **THR-SKL-9** | `worker-orchestration` | Spawn воркеров: модели, лимиты, уведомления | Не может делегировать задачи воркерам |
| **THR-SKL-10** | `openclaw-updater` | Обновление OpenClaw на всех серверах (канарейка) | Обновления без процедуры, риск поломки |
| **THR-SKL-11** | `orgrimmar-introspection` | Pre-task read, post-action update на cross-server задачах | Потеря контекста при работе с чужими серверами |

### Pipeline'ы (рабочие процессы)

| ID | Скилл | Назначение | Что сломается при удалении |
|----|-------|-----------|---------------------------|
| **THR-SKL-12** | `dev-pipeline` | SPEC → GATHER → CODE → VERIFY → REVIEW → DEPLOY | Нет структурированного процесса разработки |
| **THR-SKL-13** | `agent-bugfix` | Диагностика и починка сломанных агентов | Агенты чинятся ad-hoc, без процедуры |
| **THR-SKL-14** | `safe-update` | Раскатка изменений на агентов Sylvanas | Обновления без проверки, риск поломки |
| **THR-SKL-15** | `brainstorm-pipeline` | Структурированный ресёрч и генерация идей | -- |
| **THR-SKL-16** | `self-review` | Аудит системы по чеклисту | -- |
| **THR-SKL-17** | `pipeline-builder` | Создание новых pipeline'ов | -- |
| **THR-SKL-18** | `constitution-pipeline` | Правки конституции через PR | -- |
| **THR-SKL-19** | `auto-verify` | Автоматическая верификация после задач | -- |

### Вспомогательные (рабочие)

| ID | Скилл | Назначение | Что сломается при удалении |
|----|-------|-----------|---------------------------|
| **THR-SKL-20** | `git-workflows` | Rebase, cherry-pick, worktrees, merge conflicts | Сложные git-операции без справки |
| **THR-SKL-21** | `skill-creator` | Создание и обновление скиллов для агентов | Не может создавать новые скиллы |
| **THR-SKL-22** | `shared-memory` | Чтение shared-слоя памяти Orgrimmar | Не видит общую память команды |
| **THR-SKL-23** | `web-deploy-timeweb` | Деплой сайтов на Timeweb VPS | Дашборд и сайты не деплоятся |

### Итого: 23 защищённых скилла + github (системный OpenClaw)
