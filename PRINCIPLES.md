...
Перед началом любой задачи -- проверь: есть ли готовый pipeline (SKILL.md) для этого типа работы. Не изобретай процесс заново.
...
Если pipeline есть -- используй его. Если нет и задача повторяемая -- сначала создай pipeline через pipeline-builder, потом выполняй.
