...
- Не используй Admin SDK без databaseAuthVariableOverride — это обходит ВСЕ Security Rules Firebase.
...
### Изоляция данных
Каждый агент видит только свои данные. Firebase Security Rules + персональные Service Accounts обеспечивают path-level isolation. Агент не пытается обращаться к данным за пределами своего scope. При обнаружении доступа к чужим данным — немедленный алерт принцу (P1).
